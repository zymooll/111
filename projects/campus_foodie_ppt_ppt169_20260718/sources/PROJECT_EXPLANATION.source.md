...
Campus Foodie 是一个面向校园场景的智能饮食推荐系统。系统以“菜品或套餐”为推荐主体，结合用户主动填写的口味偏好和近期浏览、搜索、收藏等行为建立去标识化画像，再通过 DeepSeek 对数据库候选进行安全重排，帮助学生更快找到适合自己的校园餐饮。

## 2. 30 秒项目介绍

校园里的餐饮选择很多，但学生通常只能依靠熟人推荐、零散评价或反复试错。Campus Foodie 将菜品推荐、地图发现、真实评价和后台治理整合到一个系统中：用户可以直接查看个性化菜品推荐，在地图上筛选附近商家，发表带评分、文字和图片的评价；管理员可以维护商家与菜品、审核评价、导入数据并查看审计日志。FastAPI 后端负责认证、推荐、地图、评价和管理 API，DeepSeek 只负责重排数据库已经筛选出的合法候选，模型异常时系统仍可使用确定性算法正常运行。
...
- 对商家：优质菜品能够通过真实评价被发现，而不只依赖商家整体热度。
...
> 一名学生中午下课后打开 Campus Foodie。系统根据其“微辣、预算 20 元以内、常去图书馆附近”的偏好，推荐一份藤椒鸡双拼饭。学生查看菜品详情和同学评价后收藏所属商家，并在地图中查看位置。用餐后，他通过底部中央独立的“我也吃过”入口提交 5 星评价和一张图片。评价经过规则与 DeepSeek 辅助审核，不确定时进入管理员人工审核队列；发布后，其他同学的阅读会计入该用户的影响力统计。
...
配置高德 Key 时加载真实地图；未配置或加载失败时使用校园示意地图，但保留筛选、聚合和收藏语义。
...
因为用户最终需要决定的是“吃什么”。只推荐商家还需要用户再次进入菜单选择，菜品级推荐可以缩短决策链路，同时让同一商家的不同菜品根据真实评价获得不同排序。
...
先根据已发布评价计算菜品的贝叶斯平滑评分，再按菜品有效评价数量的平方根对商家下各菜品加权，避免只有少量评价的菜品对商家总分产生过大影响。
...
Campus Foodie 的核心不是简单增加一个大模型接口，而是将 AI 放在一个可验证、可降级、可审计的业务流程中。数据库保证推荐对象真实存在，确定性算法保证基础可用，DeepSeek 提升语义理解和推荐理由质量，人工审核保证内容治理，用户端与管理端共同形成完整闭环。该架构适合作为校园场景的初版产品，也为后续多校园、实时数据和更复杂推荐算法提供了清晰扩展路径。
