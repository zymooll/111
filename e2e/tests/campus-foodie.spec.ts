  ...
  for (const [index, name] of merchantNames.entries()) {
    ...
    const response = await request.post(`${apiOrigin}/admin/api/v1/merchants`, {
      ...
      data: {
        ...
        address: 'E2E 地图测试点',
        latitude: clustered ? 31.235 : 31.22,
        longitude: clustered ? 121.485 : 121.46,
        gcj02_latitude: clustered ? 31.235 : 31.22,
        gcj02_longitude: clustered ? 121.485 : 121.46,
        price_level: 2,
...
test('管理端完成商家、菜品、标签 CRUD、CSV 导入并可查询审计日志', async ({ page, request }) => {
  ...
  dialog = page.locator('.ant-modal:visible')
  await dialog.getByPlaceholder('如：学苑一食堂·风味档口').fill(merchantName)
  await chooseAntOption(page, dialog, '所属区域', '北区食堂')
  await chooseAntOption(page, dialog, '餐饮类别', '中式餐饮')
  await dialog.getByPlaceholder('用于地图定位和地点筛选').fill('北区食堂 E2E 01 号')
  ...
  await row.locator('button').first().click()
  dialog = page.locator('.ant-modal:visible')
  await dialog.getByPlaceholder('如：学苑一食堂·风味档口').fill(updatedMerchantName)
  await dialog.getByPlaceholder('用于地图定位和地点筛选').fill('北区食堂 E2E 02 号')
  ...
  const csv = [
    ...
    `${campusId},${areaId},${categoryId},${csvMerchantName},CSV E2E,北区 CSV 01 号,31.2312,121.4742,31.2312,121.4742,2,09:00-21:00`,
  ].join('\n')
