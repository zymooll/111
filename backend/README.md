...
校园饮食推荐系统的 FastAPI 初版后端。默认只依赖本地 SQLite，启动时会自动创建数据库并写入中南林业科技大学演示目录、11 家商家、96 道菜品以及开发账号；PostgreSQL、Redis 和 DeepSeek 均通过配置保留扩展接口。

学校信息来自校方官网；商家名称、地址和坐标采用 2026-07-22 查询的高德 POI 候选。菜品、价格、营业时间、描述和评价均为演示生成，不代表门店实测菜单或真实用户口碑。自动种子采用稳定 ID 和 upsert，已有演示数据库无需清空即可更新，同时不会删除用户自行创建的商家。
