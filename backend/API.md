...
推荐画像由 `/me/preferences` 的显式口味、避忌、预算与近期 `/interactions` 行为汇总生成。点击、收藏和详情阅读会按权重推断口味与常去区域；搜索词只在数据库内匹配为已知菜品标签，原始搜索文本不会发送给模型；纯曝光不参与推断，避免系统仅因自己展示过某菜品而形成反馈回路。发送给 DeepSeek 的内容只包含标签、校园区域 ID、预算和聚合信号数，不包含账号、邮箱、原始搜索文本或评价原文。
